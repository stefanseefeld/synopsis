...
void yyCheckLabelsDefinition (SymTbl *labels) {
  ...
  if (labels->clevel == FUNCTION_SCOPE &&
      labels->current) {
...
//     std::cout << " which have to be consistent" << endl ;
    ...
    if (! tagSym->entry->uStructDef ||
        tagSym->entry->uStructDef->typemask != tagType) {
...
//     std::cout << " which have to be consistent" << endl;
    ...
    else if ((tagSym->entry->uStructDef->stDefn ||
              tagSym->entry->uStructDef->enDefn) &&
...
Decl * ParseCtxt::Mk_direct_declarator_reentrance (Symbol *declSym, SymTbl *syms) {
  ...
  assert (! curCtxt->possibleDuplication) ;
...
void ParseCtxt::Mk_declarator (Decl *decl) {
  ...
  if (ident &&
    ...
    if (curCtxt->possibleDuplication) {
        ...
        assert (curCtxt->possibleDuplication->scope->level == gProject->Parse_TOS
                ->transUnit->contxt.syms->clevel) ;
        
        if (! decl->form ||
            decl->form->type != TT_Function)
          yyerr ("Duplicate function name: ", ident->name) ;
        else {
          yywarn ("TO DO: checking prototype consistency"
                  " and eventually delete the new fct symbol");
          }
        curCtxt->possibleDuplication = NULL ;
      }  
