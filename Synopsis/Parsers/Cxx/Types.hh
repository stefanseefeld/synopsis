...
public:
    ...
    //. Destructor. Note that Types::Type becomes abstract, unlike ASG::Declaration
    virtual ~Type();
...
public:
    ...
    const ASG::Declaration* declaration() const
    {
        return m_decl;
    }
...
private:
    ...
    ASG::Declaration* m_decl;
};


//. Template types are declared template types. They have a name, a
//. declaration (which is an ASG::Class) and a vector of parameters
//. declare this template. Each parameter (using ASG::Parameter) should be
...
T* declared_cast(Named* named_ptr) throw (wrong_type_cast)
{
    if (Declared* declared_ptr = dynamic_cast<Declared*>(named_ptr))
        if (ASG::Declaration* decl = declared_ptr->declaration())
