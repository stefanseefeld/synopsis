...
void SXRGenerator::write_xref(CXToken const &t)
{
  ...
  else if (c.kind == CXCursor_OverloadedDeclRef)
  {
    // A reference that hasn't yet been resolved 
    // (it may refer to a dependent type).
    // For now output the un-referenced text.
    write_esc(text);
  }
