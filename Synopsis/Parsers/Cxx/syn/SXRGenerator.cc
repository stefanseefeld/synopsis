...
SXRGenerator::SXRGenerator(FileFilter* filter, SWalker *swalker)
  : filter_(filter),
    buffer_(swalker->buffer()),
    walker_(swalker)
...
public:
  ...
  std::string describe(Types::Type* type)
  {
    ...
    {
      return sxr_->swalker()->type_formatter()->format(type);
    }
  }
...
void SXRGenerator::xref(PTree::Node *node, AST::Declaration const *decl)
{
  AST::SourceFile* file = walker_->current_file();
                              ...
                              bool continuation)
{
  ...
  AST::Scope *scope = walker_->builder()->scope();
