  ...
  try
    {
      ...
      if (m_postfix_flag == Postfix_Var)
      ...
      else
        {
          ...
          if (m_links) m_links->link(name_spec, func->declared(), LinkStore::FunctionCall);
          // Now find returnType
          m_type = func->return_type();
        }
    } 
...
SWalker::TranslateFuncall(Ptree* node) 	// and fstyle cast
{
  ...
  // m_params as a hint
  Types::Type::vector save_params = m_params;
  m_params.clear();
  try
...
SWalker::TranslateDotMember(Ptree* node)
{
  ...
  Translate(node->First());
  m_postfix_flag = save_flag;
  LOG(parse_name(node->First()) << " resolved to " << m_type_formatter->format(m_type));
  ...
  try
    ...
    { throw nodeERROR(node, "Warning: LHS of . was not a scope: " << m_type_formatter->format(m_type)); }
  // Find member, m_type becomes the var type or func returnType
...
SWalker::TranslateIf(Ptree* node) {
  ...
  m_builder->start_namespace("if", NamespaceUnique);
  // Parse expression
  Translate(node->Third());
  ...
  if (node->Length() == 7)
    {
      ...
      AST::Namespace* ns = m_builder->start_namespace("else", NamespaceUnique);
      ns->declarations().insert(
        ns->declarations().begin(), decls.begin(), decls.end());
...
SWalker::TranslateSwitch(Ptree* node)
{
  ...
  if (m_links) find_comments(node);
  if (m_links) m_links->span(node->First(), "file-keyword");
  m_builder->start_namespace("switch", NamespaceUnique);
  // Parse expression
  Translate(node->Third());
...
SWalker::TranslateFor(Ptree* node) {
  ...
  if (m_links) find_comments(node);
  if (m_links) m_links->span(node->First(), "file-keyword");
  m_builder->start_namespace("for", NamespaceUnique);
  // Parse expressions
  Translate(node->Third());
  Translate(node->Nth(3));
...
SWalker::TranslateWhile(Ptree* node)
{
  ...
  if (m_links) find_comments(node);
  if (m_links) m_links->span(node->First(), "file-keyword");
  m_builder->start_namespace("while", NamespaceUnique);
  // Parse expression
  Translate(node->Third());
  // Translate statement. If a block then we avoid starting a new ns
  Ptree* stmt = node->Nth(4);
...
SWalker::TranslateTry(Ptree* node)
{
  ...
  if (m_links) find_comments(node);
  if (m_links) m_links->span(node->First(), "file-keyword");
  m_builder->start_namespace("try", NamespaceUnique);
  Translate(node->Second());
  m_builder->end_namespace();
  for (int n = 2; n < node->Length(); n++)
    {
      ...
      m_builder->start_namespace("catch", NamespaceUnique);
          ...
          if (arg->Second() && arg->Second()->GetEncodedName())
            {
              ...
              m_builder->add_variable(m_lineno, name, arg_type, false, "exception");
            }
        }
...
Ptree*
SWalker::TranslateStorageSpecifiers(Ptree*) { STrace trace("SWalker::TranslateStorageSpecifiers NYI"); return 0; }
Ptree*
SWalker::TranslateFunctionBody(Ptree*) { STrace trace("SWalker::TranslateFunctionBody NYI"); return 0; }
...
SWalker::TranslateDo(Ptree* node)
{
  ...
  // Translate block
  m_builder->start_namespace("do", NamespaceUnique);
  // Translate statement. If a block then we avoid starting a new ns
