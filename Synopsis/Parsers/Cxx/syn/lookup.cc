...
public:
    ...
    isType(Types::Named* type) : m_value(false) { type->accept(this); }
    //. bool operator, returns the value determined by visitation during
    //. construction
    operator bool() { return m_value; }
    //. Okay
    void visit_base(Types::Base*) { m_value = true; }
    //. Okay
    void visit_unknown(Types::Unknown*) { m_value = true; }
...
AST::Function* Lookup::lookupOperator(const std::string& oper, Types::Type* left_type, Types::Type* right_type)
{
    ...
    try {
        ...
        functions.clear();
        args.clear();
    } catch (const Types::wrong_type_cast&) { /* ignore: not a class */ }
...
Types::Type* Lookup::arrayOperator(Types::Type* object, Types::Type* arg, AST::Function*& func_oper)
{
    ...
    if (info.deref) {
        ...
        // Check for typedef
        try { object = Types::declared_cast<AST::Typedef>(object)->alias(); }
        catch (const Types::wrong_type_cast&) { /* ignore -- not a typedef */ }
        // Check for modifier
    ...
    AST::Class* clas;
    try { clas = Types::declared_cast<AST::Class>(info.type); }
    catch (const Types::wrong_type_cast&) {
        TypeFormatter tf;
