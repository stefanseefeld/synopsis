...
public:
  ...
  ErrorList const &errors() const { return errors_;}
...
private:
  ...
  typedef std::pair<PTree::Node const *, SymbolTable::Symbol const *> Reference;
  typedef std::vector<Reference> ReferenceCache;
  ...
  bool is_tentative() const { return is_tentative_;}
  //. Commit to the current tentative parse.
  void commit();

  //. declare `id` to refer to symbol `s`
  ...
  bool            in_nested_name_specifier_;
  //. used for name lookup in nested-name-specifiers
  SymbolTable::Scope *qualifying_scope_;
  //.
  SymbolTable::Symbol const *symbol_;
  ...
  bool            gt_is_operator_;
  //. The template-parameter-list sequence active during a template declaration.
  TemplateParameterListSeq template_parameter_list_seq_;
