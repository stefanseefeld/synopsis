...
class Config (Base):
	...
	class CXX (Base.Parser.CXX):
	    ...
	    main_file = 1 # Dont display #included stuff
...
class PercepsCommFormatter (SummarySplitter):
    ...
    __re_comment = '^%s%s%s$'%(__c_sums,__c_dets,__c_spec) # Matches a valid comment
	...
	self.re_comment = re.compile(self.__re_comment, re.M)
	...
	mo = self.re_comment.match(comm.detail)
	if mo:
